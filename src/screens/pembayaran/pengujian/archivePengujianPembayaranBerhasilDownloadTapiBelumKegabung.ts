  ...
  const downloadQris = async () => {
    try {
      ...
      const downloadImage = async imageData => {
        const { source, filename } = imageData;

        // Pastikan URI valid
        const imageUri = source?.uri;

        if (!imageUri) {
          throw new Error(`Unable to resolve URI for ${filename}`);
        }
        ...
        // Lokasi penyimpanan untuk Android
        const downloadDestPath = `${RNFS.PicturesDirectoryPath}/${filename}`;

        // Tulis file
        await RNFS.writeFile(downloadDestPath, base64, "base64");
      ...
      const savedPaths = await Promise.all(images.map(downloadImage));

      // Alert dengan lokasi file yang disimpan
      Alert.alert(
        "Berhasil",
