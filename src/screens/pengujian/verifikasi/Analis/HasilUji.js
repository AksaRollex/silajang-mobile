        ...
        <View style={styles.headerTitleContainer}>
          ...
          <View>
            <Text style={styles.parameterTitle}>
              {parameter.nama} {parameter.keterangan ? `(${parameter.keterangan})` : ''}
            </Text>
            <Text style={styles.parameterType}>{parameter.jenis_parameter.nama}</Text>
          </View>
        </View>
      </View>
...
export default function HasilUji({ route, navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`/verifikasi/analis/${uuid}`);
  ...
  const handleParameterSubmit = async (parameterUuid, formData) => {
    try {
      const response = await axios.post(`/verifikasi/analis/${uuid}/fill-parameter`, {
