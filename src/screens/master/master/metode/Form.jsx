...
export default memo(function Form({ route, navigation }) {
  ...
  const { mutate: createOrUpdate, isLoading } = useMutation(
    (data) => axios.post(uuid ? `/master/acuan-metode/${uuid}/update` : '/master/acuan-metode/store', data),
    {
      onSuccess: () => {
        Toast.show({
          type: 'success',
    ...
    <View className="bg-[#ececec] h-full">
      <View className="bg-white rounded m-3">
        <View className="flex-row justify-between mx-3 mt-4">
          ...
          <Text className="text-xl font-poppins-semibold">{ data ? "Edit Metode" : "Tambah Metode" }</Text>
        </View>
        <View className="p-5">
