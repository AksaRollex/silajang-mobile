...
export default memo(function FormJasaPengambilan({ route, navigation }) {
  ...
  const { data, isLoading: isLoadingData } = useQuery(
    ["jasa-pengambilan", uuid],
    () =>
      uuid
        ? axios
            .get(`/master/jasa-pengambilan/${uuid}/edit`)
  ...
  const { mutate: createOrUpdate, isLoading } = useMutation(
    data =>
      axios.post(
        uuid
          ? `/master/jasa-pengambilan/${uuid}/update`
          : "/master/jasa-pengambilan/store",
        data,
      ),
    {
      onSuccess: () => {
    ...
    <View className="bg-[#ececec] h-full">
      <View
        ...
        <Text className="text-lg font-poppins-semibold ml-3">
          {data ? "Edit Jasa Pengambilan" : "Tambah Jasa Pengambilan"}
        </Text>
      </View>
      <View className="bg-white rounded m-3">
        <View className="p-5">
          ...
          />
          {errors.wilayah && (
            <Text className="text-red-500">{errors.wilayah.message}</Text>
          )}
