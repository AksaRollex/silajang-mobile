...
export default memo(function Form({ route, navigation }) {
  ...
  const { data, isLoading: isLoadingData } = useQuery(
    ...
    () =>
      name
        ? axios.get(`/master/jabatan/${name}/edit`).then(res => res.data.data)
        : null,
    {
      enabled: !!name,
      onSuccess: data => {
  ...
  const { mutate: createOrUpdate, isLoading } = useMutation(
    data =>
      axios.post(
        name ? `/master/jabatan/${name}/update` : "/master/jabatan/store",
        data,
      ),
    {
      onSuccess: () => {
    ...
    <View className="bg-[#ececec] h-full">
      <View
        ...
        <Text className="text-lg font-poppins-semibold">
          {data ? "Edit Jabatan" : "Tambah Jabatan"}
        </Text>
      </View>
      <View className="bg-white rounded m-3">
        <View className="p-5">
          ...
          />
          {errors.nama && (
            <Text className="text-red-500">{errors.full_name.message}</Text>
          )}
          <Button
