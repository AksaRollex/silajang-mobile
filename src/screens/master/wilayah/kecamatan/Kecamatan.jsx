...
const Kecamatan = ({ navigation }) => {
  ...
  const { delete: deleteKecamatan, DeleteConfirmationModal } = useDelete({
    ...
    onError: error => {
      if (
        error?.response?.data?.message?.includes("masih memiliki kelurahan")
      ) {
        Toast.show({
          type: "error",
        ...
        Toast.show({
          type: "error",
          text1: "Data Kecamatan masih memiliki kelurahan",
          text2: "Tidak dapat menghapus kecamatan yang masih memiliki kelurahan.",
        });
      }
    },
  });
    ...
    return (
      <View
        ...
        <View className="flex-row justify-end gap-2">
          <TouchableOpacity
            onPress={() =>
              navigation.navigate("FormKecamatan", { uuid: item.uuid })
            }
      ...
      <View
        ...
        <View className="flex-row items-center">
          <IonIcon
            ...
            color="#312e81"
          />
          <Text className="text-[19px] font-poppins-medium text-black ml-3">
            Kecamatan
          </Text>
        </View>
