...
export default memo(function form({ route, navigation}){
    ...
    const { data, isLoading: isLoadingData } = useQuery(['kota-kab', uuid], () => 
        uuid ? axios.get(`/master/kota-kabupaten/${uuid}/edit`).then(res => res.data.data) : null,
    {
        enabled: !!uuid, 
        onSuccess: (data) => {
            if (data) {
    ...
    const { mutate: createOrUpdate, isLoading } = useMutation(
        (data) => axios.post(uuid ? `/master/kota-kabupaten/${uuid}/update` : '/master/kota-kabupaten/store', data),    
    {
        onSuccess: () => {
            Toast.show({
                type: "success",
        ...
        <ScrollView className="bg-[#ececec] h-full">
            <View className="bg-white rounded m-3">
                <View className="flex-row justify-between mx-3 mt-4">
                    ...
                    <Text className="text-xl font-poppins-semibold">{ data ? "Edit Kota/Kabupaten" : "Tambah Kota/kabupaten" }</Text>
                </View>
                <View className="p-5">
